...
type Node struct {
	label  		string 		//The label for the node for consistent numbering
...
func main() {
	...
	PrintTreeToNewick(T, T[0])
	NewickString += ";"
	NewickToTXT("NewickTest")
	
}


func PrintTreeToNewick(T Tree, root *Node) {	
	NewickString += "("
	for i:=range(root.children) {
		if len(root.children[i].children) == 0 {
			...
			NewickString += root.children[i].label
			MakeAndPrintMaps(root.children[i])
		} else {
			PrintTreeToNewick(T, root.children[i])
		}
	}
	NewickString += ")" + root.label
}
...
func NewickToTXT(filename string) {
	...
	fmt.Fprint(outFile,NewickString)
	outFile.Close()
}
...
func HeirarchalCluster(HSet []TagInfo, d int, dims int, T *Tree) TagInfo {
	...
	var newLabel string
	for j := range(newTag) {
		newLabel = newLabel + strconv.Itoa(newTag[j]) + "/"
	}
	if (len(HSet[0].label)) == 1 {
		newLabel += "Origin"
	} else {
	newLabel += "I"
	}
...
func InitializeTree(speciesNames []string) Tree {
	...
	for i := range t {
		...
		} //set internal node names to something
