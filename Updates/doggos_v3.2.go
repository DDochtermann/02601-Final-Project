...
func main() {
	...
	tagLevel := 1
	var strtag string = "0"
	RecursiveBreakdown(&FinalSet, PupMatrices, locusNames, dimensions, maxClusterSize, tagLevel, strtag)
	/*
	var sum int
	for i:=range(FinalSet) {
		sum+=len(FinalSet[i])
		fmt.Println("Cluster", i, "contains", len(FinalSet[i]), "points")
	}	
	fmt.Println(sum)
...
func RecursiveBreakdown(FinalSet *[][]Pup, PupMatrices [][]Pup, locusNames []string, dimensions int, maxSize int, tagLevel int, originTag string)  [][]Pup {
	for i:= range(PupMatrices) {
		if len(PupMatrices[i]) <= maxSize {
			if len(PupMatrices[i]) > 0 {
				*FinalSet = append(*FinalSet, PupMatrices[i])
				fmt.Println(originTag, tagLevel, i, "complete (", len(PupMatrices[i]), ")")
			}
		} else {
			fmt.Println(originTag, tagLevel, i, "being broken down (", len(PupMatrices[i]), ")")	
			newtag := originTag + " " + strconv.Itoa(tagLevel)
			...
			RecursiveBreakdown(FinalSet, NewPupMatrices, locusNames, dimensions, maxSize, i, newtag)
		}
	}
	return *FinalSet
}
