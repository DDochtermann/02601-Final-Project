...
func main() {
	...
	fmt.Println("Total number of clusters:", len(FinalSet))
	for i := range FinalSet {
		fmt.Println("set", FinalTags[i], "is length", len(FinalSet[i]))
	}
	...
	for i := currentDepth; i > 0; i-- {
		fmt.Println("entering this loop with i=", i)
		HSet = append(GetSetsAtDepths(scores, FinalSet, FinalTags, i, dimensions), nextHsets...)
		nextHsets = AHC(HSet, i)
		for j:= range(HSet) {
			fmt.Print(HSet[j].label, " ")
			fmt.Println()
		}
	}

	fmt.Println(len(nextHsets))
	//currentDepth-- in some loop, decreasing. Heirarch the returned HSets by second
	//to last index, reduce []int by one, add them to an HSet, retrieve the
	//rest from the main set
...
func AHC(HSet []TagInfo, d int) []TagInfo {
	var toDo []int
	var x int
	var matches int
	var compoundedHSets []TagInfo
	//collect levels that will be grouped
	if d == 1 {
		compoundedHSets = append(compoundedHSets, HeirarchalCluster(HSet, d))
	} 
	if d!=1 {
		...
		for i := range toDo {
			...
			compoundedHSets = append(compoundedHSets, HeirarchalCluster(toCluster, d))
		}
	}
...
func HeirarchalCluster(HSet []TagInfo, d int) TagInfo {
